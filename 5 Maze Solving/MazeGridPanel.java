...
public class MazeGridPanel extends JPanel{
	...
	public void solveMaze() {
		...
		stack.push(start);
		while(!stack.empty()) {
			Cell currentPosition = stack.peek();
			if (currentPosition.getBackground() == Color.RED) {
				break;
			}
			if (!currentPosition.northWall && !visited(currentPosition.row-1, currentPosition.col)) {
				...
				currentPosition.setBackground(Color.CYAN);
			}
				...
				currentPosition.setBackground(Color.PINK);
				stack.pop();
			}
		}
		start.setBackground(Color.GREEN);
